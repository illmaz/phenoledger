...
export async function downloadMonthlySummary(year, month) {
  const res = await apiFetch(`${BASE}/reports/monthly-summary?year=${encodeURIComponent(year)}&month=${encodeURIComponent(month)}`, {
    headers: { ...await authHeaders() }
  })
  if (!res.ok) {
    const body = await res.json().catch(() => ({}))
  ...
  a.href = url
  a.download = `MonthlySummary_${year}_${String(month).padStart(2, '0')}.pdf`
  a.click()
...
export async function createExportRecord(data) {
  const res = await apiFetch(`${BASE}/export-records`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...await authHeaders() },
    body: JSON.stringify(data),
  })
...
export async function deleteExportRecord(id) {
  const res = await apiFetch(`${BASE}/export-records/${encodeURIComponent(id)}`, {
    method: 'DELETE',
    headers: await authHeaders(),
  })
  if (!res.ok) { const b = await res.json().catch(() => ({})); throw new Error(b.detail || res.status) }
